...
class detect_dif_situation():
    ...
    def detect_dif_up(self,df, lineEdit):
        ...
        if (cross_time>1):
            if ((df['dif'][cross_time_list[1] - 1] > 0) & (df['dif'][cross_time_list[1]] < 0)):
                lineEdit.append('过去' + str(detect_period) + '天，在'+str(df['trade_date'][cross_time_list[1]])+'第二次穿过0轴')
        if (cross_time == 3):
            if ((df['dif'][cross_time_list[2]-1] < 0) & (df['dif'][cross_time_list[2]] > 0)):
                lineEdit.append('过去' + str(detect_period) + '天，在'+str(df['trade_date'][cross_time_list[2]])+'第三次突破0轴（可能是第二买点）')
        if cross_time > 3:
            pass
    ...
    def detect_dif_down(self,df,lineEdit):
       #检测是否过去detect_period天是否突破0轴（寻找卖点）
        ...
        if (cross_time >= 2):
            if ((df['dif'][cross_time_list[1]-1] < 0) & (df['dif'][cross_time_list[1]] > 0)):
                lineEdit.append('过去' + str(detect_period) + '天，在'+str(df['trade_date'][cross_time_list[1]])+'第二次突破0轴')
        if (cross_time == 3):
            if ((df['dif'][cross_time_list[2]-1] > 0) & (df['dif'][cross_time_list[2]] < 0)):
                lineEdit.append('过去' + str(detect_period) + '天，在'+str(df['trade_date'][cross_time_list[2]])+'第三次穿过0轴（可能是第二卖点）')
        if cross_time > 3:
            pass
...
class trend_and_dif():
    def detect_peak_and_valley(self,df):
        ...
        indices_valley = signal.find_peaks(np.array([-i for i in smooth_signal])[-detect_trend_period:], height=None, threshold=None, distance=5,
                             prominence=None, width=None, wlen=None, rel_height=None,
                             plateau_size=None)
    ...
    def detect_break_cross_trend(self,df,lineEdit):
        ...
        #检测是否跌破上升趋势线
        k1, b1 = self.solve_equation([valley_list[-1], df['dif'][valley_list[-1]]], [valley_list[-2], df['dif'][valley_list[-2]]])
        break_point = 0
...
class detect_divergence():
    ...
    def detect_bottom_divergence(self,df,lineEdit):
        ...
        k_dif, b_dif = self.solve_equation([valley_list_dif[-1],df['dif'][valley_list_dif[-1]]],[valley_list_dif[-2],df['dif'][valley_list_dif[-2]]])
        k_close, b_close = self.solve_equation([valley_list_close[-1], df['close'][valley_list_close[-1]]],[valley_list_close[-2], df['close'][valley_list_close[-2]]])
        first_buy_point = 0
        if (k_dif>0) & (k_close < 0):
            first_buy_point = valley_list_dif[-1]+1
        if first_buy_point!=0:
            lineEdit.append('过去'+str(detect_trend_period)+'天，发现DIF与收盘价的底背离，发生在'+str(df['trade_date'][first_buy_point])+'(可能是第一买点)')
    ...
    def detect_top_divergence(self,df,lineEdit):
        ...
        k_dif, b_dif = self.solve_equation([peak_list_dif[-1],df['dif'][peak_list_dif[-1]]],[peak_list_dif[-2],df['dif'][peak_list_dif[-2]]])
        k_close, b_close = self.solve_equation([peak_list_close[-1], df['close'][peak_list_close[-1]]],[peak_list_close[-2], df['close'][peak_list_close[-2]]])
        first_sell_point = 0
        if (k_dif < 0) & (k_close > 0):
            first_sell_point = peak_list_dif[-1]+1
        if first_sell_point != 0:
            lineEdit.append('过去'+str(detect_trend_period)+'天，发现DIF与收盘价的顶背离，发生在'+str(df['trade_date'][first_sell_point])+'(可能是第一卖点)')
