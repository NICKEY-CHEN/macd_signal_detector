...
class detect_macd_histogram():
    def detect_hist_peak(self,df):
        ...
        for i in range(len(df)-detect_histogram_period,len(df)-1):
            if (df['hist'][i]>0) & (df['hist'][i-1]>0) & (df['hist'][i+1]>0):
                if ((df['hist'][i] - df['hist'][i-1])>df['hist'][i-1]*higher_rate_hist_peak) & ((df['hist'][i] - df['hist'][i+1])>df['hist'][i+1]*higher_rate_hist_peak):
    ...
    def detect_hist_valley(self,df):
        ...
        for i in range(len(df)-detect_histogram_period,len(df)-1):
            if (df['hist'][i]<0) & (df['hist'][i-1]<0) & (df['hist'][i+1]<0):
                if ((df['hist'][i] - df['hist'][i-1])<df['hist'][i-1]*higher_rate_hist_valley) & ((df['hist'][i] - df['hist'][i+1])<df['hist'][i+1]*higher_rate_hist_valley):
    ...
    def detect_pull_feet(self,df,lineEdit):
        ...
        # print()
        lineEdit.append('在'+str([df['trade_date'][i+1] for i in valley_list_hist])+'处发生抽脚，<font color=\"#CD5C5C\">Tips</font>:DIF线在0轴上方和0轴附近的成功率较高。“绿柱变红柱”买点比“绿柱抽脚”买点成功率高且更稳健。需要同时伴随成交量明显放大。')

    #判断缩头
    def detect_shrink_head(self,df,lineEdit):
        peak_list_hist = self.detect_hist_peak(df)
        lineEdit.append('在' + str([df['trade_date'][i+1] for i in peak_list_hist]) + '处发生缩头，<font color=\"#CD5C5C\">Tips</font>:多头市场的缩头可以看作涨势在减弱，但不一定会跌；而空头市场的缩头通常是新的一轮下跌的开始。')
    ...
    def detect_top_divergence(self,df,lineEdit):
        ...
        for i in range(len(df)-detect_histogram_period,len(df)-1):
            if (df['hist'][i]>0) & (df['hist'][i-1]>0) & (df['hist'][i+1]>0):
                if ((df['hist'][i] - df['hist'][i-1])>df['hist'][i-1]*0.03) & ((df['hist'][i] - df['hist'][i+1])>df['hist'][i+1]*0.03):
                    if df['hist'][i] > 0.2*df['hist'].max():
                        hist_peak_list.append(i)
    ...
    def detect_bottom_divergence(self,df,lineEdit):
        ...
        for i in range(len(df)-detect_histogram_period,len(df)-1):
            if (df['hist'][i]<0) & (df['hist'][i-1]<0) & (df['hist'][i+1]<0):
                if ((df['hist'][i] - df['hist'][i-1])<df['hist'][i-1]*0.05) & ((df['hist'][i] - df['hist'][i+1])<df['hist'][i+1]*0.05):
                    if df['hist'][i] < 0.2*df['hist'].min():
                        hist_valley_list.append(i)
        ...
        if len(valley_list_hist)>0:
            for i in range(len(valley_list_hist)-1):
                if np.array([i for i in df['hist'][valley_list_hist[i]:valley_list_hist[i+1]]]).max()<0:
                    kill_long_list.append(valley_list_hist[i]+np.argmax(np.array([i for i in df['hist'][valley_list_hist[i]:valley_list_hist[i+1]]])))
                    lineEdit.append('在'+str(df['trade_date'][valley_list_hist[i]+np.argmax(np.array([i for i in df['hist'][valley_list_hist[i]:valley_list_hist[i+1]]]))])+'处出现\'<font color=\"#DAA520\">杀多棒</font>\'')
        ...
        if len(peak_list_hist)>0:
            for i in range(len(peak_list_hist)-1):
                if np.array([i for i in df['hist'][peak_list_hist[i]:peak_list_hist[i+1]]]).min()>0:
                    force_short_list.append(peak_list_hist[i]+np.argmax(np.array([i for i in df['hist'][peak_list_hist[i]:peak_list_hist[i+1]]])))
                    lineEdit.append('在'+str(df['trade_date'][peak_list_hist[i]+np.argmax(np.array([i for i in df['hist'][peak_list_hist[i]:peak_list_hist[i+1]]]))])+'处出现\'<font color=\"#DAA520\">逼空棒</font>\'')
...
#     # DIF寻找买点
#     macd_histogram.detect_pull_feet(df4)
#     macd_histogram.detect_shrink_head(df4)
#     print('-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -')
#     macd_histogram.detect_top_divergence(df4)
#     macd_histogram.detect_bottom_divergence(df4)
#     print('-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -')
#     macd_histogram.detect_kill_long_bin(df4)
#     macd_histogram.detect_force_short_bin(df4)
#     print('-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -')